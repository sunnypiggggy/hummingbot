...
def test_aug10_live_dust_snapshot_migrates_without_sell_or_repeat_episode(tmp_path):
    ledger = UnifiedInventoryLedger(tmp_path)
    for asset, quantity in (
        ("BTC", "0.000004857672861421803102676728"),
        ("ETH", "0.00227157909293506300096269939"),
    ):
        ledger.start_job(
            job_id=f"existing-{asset}", asset=asset, scope="unattributed_dust",
            pair=f"{asset}-USDT", requested_quantity=Decimal(quantity),
            client_order_id=f"inv-existing-{asset}", now=1,
        )
        ledger.finish_job(
            f"existing-{asset}", status="DUST", error="below exchange minimum", now=2,
        )
    balances = {
        "BTC": {"free": "0.00021462", "locked": "0.00300000", "total": "0.00321462"},
        "ETH": {"free": "0.00880000", "locked": "0.10110000", "total": "0.10990000"},
    }
    ownership = {
        "BTC": {
            "dca:dca-live-btcusdt-200": "0.001499762327138578196897323272",
            "grid:grid-live-fdusd-400": "0.00171000",
        },
        "ETH": {
            "dca:dca-live-ethusdt-200": "0.05052842090706493699903730061",
            "grid:grid-live-fdusd-400": "0.05710000",
        },
    }
    first = ledger.reconcile(
        account_fingerprint="aug10-account", balances=balances, ownership=ownership,
        evidence_sha256="dynamic-one",
        open_order_counts={"BTC-FDUSD": 4, "BTC-USDT": 8,
                           "ETH-FDUSD": 4, "ETH-USDT": 8},
        sources_healthy=True, now=100,
    )
    second = ledger.reconcile(
        account_fingerprint="aug10-account", balances=balances, ownership=ownership,
        evidence_sha256="dynamic-two",
        open_order_counts={"BTC-FDUSD": 4, "BTC-USDT": 8,
                           "ETH-FDUSD": 4, "ETH-USDT": 8},
        sources_healthy=True, now=140,
    )
    assert first["healthy"] is False and second["healthy"] is False
    for asset in ("BTC", "ETH"):
        assert first["assets"][asset]["inventory_phase"] == "DUST"
        assert second["assets"][asset]["inventory_phase"] == "DUST"
        assert second["assets"][asset]["confirmation"]["cycles"] == 0
        assert first["assets"][asset]["episode_id"] == second["assets"][asset]["episode_id"]
    with ledger._connection() as connection:
        assert connection.execute("SELECT COUNT(*) FROM liquidation_jobs").fetchone()[0] == 2
