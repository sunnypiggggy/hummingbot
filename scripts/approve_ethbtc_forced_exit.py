...
OBSERVATION_WAIVER_CONFIRMATION = "WAIVE-ETHBTC-24H-OBSERVATION"
...
def checked_observation(args: argparse.Namespace, release_sha: str) -> dict:
    payload = json.loads(args.observation_report.read_text(encoding="utf-8"))
    if (
        payload.get("schema") != "ethbtc-forced-exit-observation-v1"
        or payload.get("release_sha256") != release_sha
    ):
        raise ValueError("invalid ethbtc-forced-exit-observation-v1 evidence")
    if payload.get("passed") is True:
        return payload
    checks = dict(payload.get("checks", {}))
    duration_is_only_failure = (
        checks.get("duration_24h") is False
        and all(value is True for key, value in checks.items() if key != "duration_24h")
    )
    if not (
        args.waive_observation_duration
        and args.waiver_confirm == OBSERVATION_WAIVER_CONFIRMATION
        and str(args.waiver_reason).strip()
        and duration_is_only_failure
    ):
        raise ValueError("observation did not pass and has no valid duration-only waiver")
    return payload
...
def approve(args: argparse.Namespace) -> int:
    ...
    observation = checked_observation(args, release_sha)
    observed_seconds = int(observation["ended_at"]) - int(observation["started_at"])
    if observed_seconds < 86400 and not args.waive_observation_duration:
    ...
    receipt = {
        ...
        "observation_duration_waived": bool(args.waive_observation_duration),
        "observation_waiver_reason": (
            str(args.waiver_reason).strip() if args.waive_observation_duration else None
        ),
        "observation_waiver_confirmation": (
            OBSERVATION_WAIVER_CONFIRMATION if args.waive_observation_duration else None
        ),
...
def main() -> int:
    ...
    approved.add_argument("--waive-observation-duration", action="store_true")
    approved.add_argument("--waiver-reason", default="")
    approved.add_argument("--waiver-confirm", default="")
