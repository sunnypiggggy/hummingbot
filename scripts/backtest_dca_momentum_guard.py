#!/usr/bin/env python3
"""Backtest the live BTC/ETH DCA with stateful ROC/SQZMOM risk gates.
...
SCENARIOS = ("baseline", "roc", "sqzmom", "combined")
...
def gate_for_frame(frame: pd.DataFrame, signals: pd.DataFrame, scenario: str) -> pd.Series:
    ...
    values = schedule.reindex(targets, method="ffill").fillna(True).astype(bool).to_numpy()
...
def combine_scenario(pair_curves: dict[str, pd.DataFrame], pair_summaries: list[dict], scenario: str) -> tuple[dict, pd.DataFrame]:
    ...
    summary = {
        ...
        "enabled_pct": pair_summaries[0]["enabled_pct"],
    }
...
def make_figure(signals: pd.DataFrame, curves: dict[str, pd.DataFrame], periods: pd.DataFrame,
    ...
    palette = {"baseline": "#4b5563", "roc": "#d97706", "sqzmom": "#2563eb", "combined": "#111827"}
    labels = {"baseline": "无指标风控", "roc": "ROC 风控", "sqzmom": "SQZMOM 风控", "combined": "组合风控"}
    dashes = {"baseline": "dot", "roc": "solid", "sqzmom": "solid", "combined": "dash"}
...
def main() -> int:
    ...
    for scenario in SCENARIOS:
        ...
        for pair, frame in frames.items():
            gate = gate_for_frame(frame, signals, scenario)
