...
class BotStore:
    def __init__(self, path: Path, session_ttl_seconds: int = 900):
        ...
        self.db.executescript(
            ...
            CREATE TABLE IF NOT EXISTS stock_schedule_subscriptions(
              schedule_id TEXT PRIMARY KEY,user_id INTEGER NOT NULL,chat_id INTEGER NOT NULL,
              last_status TEXT NOT NULL DEFAULT '',last_version INTEGER NOT NULL DEFAULT 0,
              executor_id TEXT,created_at REAL NOT NULL,updated_at REAL NOT NULL
            );
            """
        )
        self.db.commit()

    def subscribe_stock_schedule(self, schedule_id: str, user_id: int, chat_id: int) -> None:
        now = time.time()
        self.db.execute(
            "INSERT INTO stock_schedule_subscriptions(schedule_id,user_id,chat_id,created_at,updated_at) "
            "VALUES(?,?,?,?,?) ON CONFLICT(schedule_id) DO UPDATE SET user_id=excluded.user_id,"
            "chat_id=excluded.chat_id,updated_at=excluded.updated_at",
            (schedule_id, int(user_id), int(chat_id), now, now),
        )
        self.db.commit()

    def stock_schedule_subscriptions(self) -> list[dict]:
        return [dict(row) for row in self.db.execute(
            "SELECT * FROM stock_schedule_subscriptions ORDER BY created_at"
        ).fetchall()]

    def mark_stock_schedule_notified(
        self, schedule_id: str, status: str, version: int, executor_id: str = ""
    ) -> None:
        self.db.execute(
            "UPDATE stock_schedule_subscriptions SET last_status=?,last_version=?,executor_id=?,updated_at=? "
            "WHERE schedule_id=?",
            (status, int(version), executor_id or None, time.time(), schedule_id),
        )
        self.db.commit()
