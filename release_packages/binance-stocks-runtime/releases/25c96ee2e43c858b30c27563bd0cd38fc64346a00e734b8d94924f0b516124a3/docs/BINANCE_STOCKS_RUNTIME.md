...
- `IMMEDIATE`（默认）：保持兼容；若时段不合格，由 Connector 在创建阶段拒绝。
- `QUEUE_IF_CLOSED`：时段合格时立即创建，否则先写入 PostgreSQL 计划单。

LIMIT 固定确认时的价格和股数，目标时段为下一次 `PRE_MARKET/MARKET_OPEN/POST_MARKET`。
BUY MARKET 和 Position MARKET 固定 `quote_budget`，只在 `MARKET_OPEN` 使用最新 Ask 和
动态数量精度重新计算股数；SELL MARKET 固定归属股数。排队事务会同时预留 BUY 预算与
费用，或锁定 SELL 管理库存。取消、过期及硬拒绝会原子释放预留。

状态机为：

```text
QUEUED -> WAITING_SESSION -> WAITING_PREFLIGHT -> ACTIVATING -> ACTIVE
                                                     \-> CANCELED/EXPIRED/REJECTED/FAILED
```

行情、市场状态或 API 暂时不可信时继续排队并退避重试，不创建 Executor。第一次观察到
合格时段后绑定 Binance `tradingDate`；该交易日结束仍未激活则过期。始终无法获得可信
日历时使用 7 天绝对安全上限。Position 的止损、止盈、追踪和持仓计时仍从首次成交开始。
