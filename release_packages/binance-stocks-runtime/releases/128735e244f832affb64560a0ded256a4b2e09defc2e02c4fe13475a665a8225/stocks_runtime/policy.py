...
class StocksExecutorPolicy:
    ...
    async def _validate(
        ...
        *,
        ...
        side = self._enum(config.get("side"))
        ...
        if executor_type == "order_executor":
            strategy = self._enum(config.get("execution_strategy"))
            ...
            for field in ("open_order_type", "take_profit_order_type"):
                value = self._enum(barriers.get(field, "LIMIT"))
        ...
        common = {
            ...
            "would_submit": self.mode == "LIVE" and self.live_authorized,
