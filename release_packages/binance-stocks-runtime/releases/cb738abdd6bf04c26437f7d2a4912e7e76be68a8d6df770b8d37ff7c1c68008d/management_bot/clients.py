...
class StocksClient(JsonClient):
    ...
    def scheduled(self, active_only: bool = True) -> list[dict]:
        value = self.request(
            "GET", f"/stocks/scheduled-executors?active_only={'true' if active_only else 'false'}"
        )
        return list(value.get("items", [])) if isinstance(value, dict) else []

    def scheduled_detail(self, schedule_id: str) -> dict:
        value = self.request("GET", f"/stocks/scheduled-executors/{schedule_id}")
        return value if isinstance(value, dict) else {}

    def refresh_scheduled(self, schedule_id: str) -> dict:
        value = self.request("POST", f"/stocks/scheduled-executors/{schedule_id}/refresh")
        return value if isinstance(value, dict) else {}

    def cancel_scheduled(self, schedule_id: str) -> dict:
        value = self.request("POST", f"/stocks/scheduled-executors/{schedule_id}/cancel")
        return value if isinstance(value, dict) else {}
