...
class BinanceStocksPositionExecutor(PositionExecutor):
    ...
    def paper_checkpoint_state(self):
        return {
            ...
            "open_order_id": self._open_order.order_id if self._open_order else None,
            "close_order_id": self._close_order.order_id if self._close_order else None,
            "take_profit_order_id": (
                self._take_profit_limit_order.order_id if self._take_profit_limit_order else None
            ),
            "first_fill_timestamp": self._first_fill_timestamp,
            "pending_close_type": self._pending_close_type.name if self._pending_close_type else None,
            "close_order_extended": self._close_order_extended,
            "close_order_submitted_at": self._close_order_submitted_at,
            "entry_filled_backup": str(self._entry_filled_backup),
            "entry_quote_backup": str(self._entry_quote_backup),
            "exit_filled_backup": str(self._exit_filled_backup),
            "exit_quote_backup": str(self._exit_quote_backup),
            "fees_quote_backup": str(self._fees_quote_backup),
    ...
    def restore_paper_checkpoint(self, state):
        ...
        self._take_profit_limit_order = tracked(state.get("take_profit_order_id"))
        self._first_fill_timestamp = state.get("first_fill_timestamp")
        pending = state.get("pending_close_type")
        self._pending_close_type = CloseType[pending] if pending in CloseType.__members__ else None
        self._close_order_extended = bool(state.get("close_order_extended", False))
        self._close_order_submitted_at = float(state.get("close_order_submitted_at", 0))
        self._entry_filled_backup = Decimal(str(state.get("entry_filled_backup", "0")))
        self._entry_quote_backup = Decimal(str(state.get("entry_quote_backup", "0")))
        self._exit_filled_backup = Decimal(str(state.get("exit_filled_backup", "0")))
        self._exit_quote_backup = Decimal(str(state.get("exit_quote_backup", "0")))
        self._fees_quote_backup = Decimal(str(state.get("fees_quote_backup", "0")))
