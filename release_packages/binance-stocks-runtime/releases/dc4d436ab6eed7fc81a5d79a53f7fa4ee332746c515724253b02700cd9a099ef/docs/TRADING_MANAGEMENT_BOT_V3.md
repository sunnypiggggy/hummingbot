...
活动限额保存于Stock Postgres：单笔本金、单股票本金/MTM、Stock总本金/MTM和日亏损。
不存在Compose硬上限；Telegram修改直接写PostgreSQL，最终范围由可信账户资金决定。
运行时必须满足：
...
预检业务不通过返回正常响应并保留向导，展示请求本金、当前占用、费用预留和可用额度，
可选择“修改金额/修改限额/取消”。HTTP 409仅用于同一ID不同配置或取消/激活并发；
格式错误为422，行情或数据库暂时不可用为503。
