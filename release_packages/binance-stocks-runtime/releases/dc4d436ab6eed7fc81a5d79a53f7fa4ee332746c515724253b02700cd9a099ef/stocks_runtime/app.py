...
def _install_executor_policy(app) -> None:
    ...
    async def guarded_create(executor_config, account_name=None, controller_id=None):
        executor_config = normalize_executor_config(executor_config)
    ...
    # The scheduler calls this only after its durable reservation has been
    # atomically claimed.  Calling guarded_create here would reserve twice.
    app.state.stocks_raw_create = original_create
...
async def stocks_lifespan(runtime_app):
    ...
    if settings.scenario_mode:
        from hummingbot.connector.exchange.binance_stocks import binance_stocks_constants as stocks_constants
        stocks_constants.REST_URL = settings.rest_url
        stocks_constants.WS_URL = settings.ws_url
    ...
    try:
            ...
            def paper_connector_factory(account_name, connector_name):
                if account_name == settings.account_name and connector_name == "binance_stocks":
                    return BinanceStocksPaperExchange(
                        ...
                        time_in_force="DAY",
                        # A non-empty, operator-controlled list is required for
                        # ExchangePyBase to start the public market-state stream.
                        # This subscribes calendar plus only the whitelist's
                        # Quote/status/tradability channels, never all tickers.
                        trading_pairs=whitelist_pairs,
        ...
        runtime_app.state.stocks_async_scheduler = AsyncStocksOrderScheduler(runtime_app)
