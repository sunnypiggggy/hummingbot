...
class BinanceStocksPaperExchange(BinanceStocksExchange):
    ...
    def is_cancel_request_in_exchange_synchronous(self) -> bool:
        # The real Stocks API only acknowledges cancel requests and remains
        # asynchronous. The local paper venue commits the terminal state in
        # PostgreSQL before returning, so waiting for an orderReport that can
        # never exist would strand the Executor in SHUTTING_DOWN.
        return True
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        canceled = await self.paper_broker.cancel_order(order_id)
        if canceled:
            row = await self.paper_broker.order(order_id)
            await self.paper_broker.ledger.record_cumulative_fill(
                client_order_id=order_id,
