...
ACTIVE_INTENT_STATES = {"RESERVED", "RUNNING", "EXIT_PENDING", "CLOSING", "POSITION_HOLD"}
...
class PostgresManagedLedger:
    ...
    async def managed_symbol_exposure(self, symbol: str, price: Decimal) -> Decimal:
        async with self._pool.acquire() as connection:
    ...
    async def reserve_intent(
        ...
        *,
        ...
        return {"executor_id": executor_id, "status": "RESERVED"}
    ...
    async def managed_exposure(self, prices: Dict[str, Decimal]) -> Decimal:
            ...
            async with connection.transaction():
                ...
                if delta_base > 0:
                    if order["side"] == "BUY":
                        ...
                        # Direct SELL intents reserve available_base up front. Internal
                        # PositionExecutor exits do not, so consume availability here.
                        if intent["side"] != "SELL" or intent["source_owner"] is None:
                            await connection.execute(
