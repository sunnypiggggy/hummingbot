...
v22 候选生成中文 PDF，以及 Grid BTC、Grid ETH、DCA BTC、DCA ETH 四张独立
PNG；Grid 参数候选只生成两张 Grid PNG。候选与激活在同一 Grid 调度事务时合并
为一个激活事件。

360 天窗口为截止时间前最后 360 个完整 UTC 自然日。图片展示价格、单机器人
连续权益、回撤、v22 概率和逐周阈值；黄色区间表示 v22 Risk-Off，进入和恢复
使用不同标记，签名模型未覆盖区间使用红色边框并按 Fail-Closed 纯现金展示。

只有哈希匹配的回放证据才能生成 PNG。本地历史不足时从 Binance 回补价格并校验
连续性；模型回放证据缺失或哈希不匹配时，不推测信号、不伪造图片，只发送标为
“证据缺失”的 PDF 和严重告警。该失败不改变 v22 当前周、哈希、观察期和人工
审批门槛，也不阻断本来已满足旧部署门槛的 Grid 参数更新。
...
3. 重建现有 `dca-live-report`、Grid Guard、DCA Guard 和 Grid scheduler 镜像，
   不新增服务。
4. 先保持总开关关闭，检查事件 JSONL、outbox、四张测试 PNG 和目标频道 ID。
