...
try:
    from scripts.telegram_notifications import RuntimeErrorChannel
...
class V2WithControllers(StrategyV2Base):
    ...
    def __init__(self, connectors: Dict[str, ConnectorBase], config: V2WithControllersConfig):
        ...
        config_name = str(os.getenv("SCRIPT_CONFIG", "dca-live"))
        bot_name = Path(config_name).stem
        pair = "UNKNOWN"
        if "btcusdt" in bot_name.lower():
            pair = "BTC-USDT"
        elif "ethusdt" in bot_name.lower():
            pair = "ETH-USDT"
        error_root = Path("data")
        self.runtime_errors = RuntimeErrorChannel(
            ...
            source="dca-live-strategy", strategy="dca", bot=bot_name, pair=pair,
        )

    def on_tick(self):
        self.runtime_errors.recover_if_quiet(
            "order_submission", quiet_seconds=300,
            trading_status="连续5分钟无新的 DCA 订单拒绝；executor 继续按聚合风控门运行",
    ...
    def did_fail_order(self, order_failed_event: MarketOrderFailureEvent):
        ...
        """
        summary = (
            order_failed_event.error_message or order_failed_event.error_type
            or "exchange rejected order"
        )
        self.runtime_errors.failure(
            "order_submission", summary,
            trading_impact="该订单未成交；DCA executor 和聚合风控门继续决定后续交易。",
            severity="warning", action="executor_reconcile_then_retry_if_allowed",
            details={
                "order_id": str(order_failed_event.order_id)[:32],
                "order_type": str(order_failed_event.order_type),
            },
            now=float(order_failed_event.timestamp),
        )
