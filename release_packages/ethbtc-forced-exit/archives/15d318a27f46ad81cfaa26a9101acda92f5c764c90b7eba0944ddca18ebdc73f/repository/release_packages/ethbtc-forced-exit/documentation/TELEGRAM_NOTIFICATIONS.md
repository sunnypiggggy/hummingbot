...
运行错误使用 `runtime_error` 机制以及 `ERROR_OCCURRED`、`ERROR_RECOVERED`
两个转换。`grid-live-guard` 和 `dca-live-guard` 监控自身主循环，同时通过 Docker
只读日志接口扫描三个交易机器人；`dca-live-report` 监控报告数据源、报告主循环及
Telegram 持久化发送队列。因此，Guard 周期异常、交易机器人日志中的订单拒绝或
执行异常、收益报告降级以及频道投递失败都可以在频道呈现。

每条告警必须明确列出组件、机器人、交易对、经过脱敏的错误摘要、当前影响、自动
动作、UTC 时间和事件 ID。Token、API key、密码、签名参数和 Telegram Bot URL
在进入事件文件前即被替换为 `[REDACTED]`；摘要最长 600 个字符。

`RuntimeErrorChannel` 将同一组件与同一错误指纹形成一个持久化 episode。错误持续发生时只累计次数，不按
Guard 循环刷屏；进程重启后仍沿用原 episode。错误消失或机器人日志连续 5 分钟无
同类错误时发送一次恢复消息，包含持续时间、累计失败次数和恢复后的实际状态。新
错误指纹开启新 episode。

运行错误通知本身不改变交易权限：

- Guard 主循环短暂失败会自动重试；是否以及何时 Fail-Closed，继续由原有连续失败
  阈值和完整性熔断控制。
- 交易机器人日志错误只用于及时暴露异常；BUY/SELL 是否放行仍由该机器人当前聚合
  风控门决定。
- 日志采集失败只降低日志可见性，不得使 Guard 主循环失败或改变交易开关。
- 报告数据缺失、图片失败和 Telegram 投递失败不影响交易；outbox 持久化重试。

运行错误状态分别保存在 Guard/Report 的可写状态目录，交易机器人不持有 Telegram
Token。`dca-live-report` 仍是唯一发送器，事件按事件 ID、附件哈希和频道 ID 幂等
去重。验收必须同时检查：三项服务健康且无重启、交易机器人容器 ID 未变化、日志
游标持续前进、outbox 没有 `pending/retrying`，以及测试错误与恢复消息均取得真实的
Telegram message ID。
...
参数或模型候选产生后，生产者只能发布 `PARAMETER_APPROVAL_PENDING` 或
`MODEL_APPROVAL_PENDING`，不能直接发布激活合同。报告服务生成并发送证据后，必须等
Telegram 对全部附件返回 message ID，才在
`dca-live-data/telegram/evidence_receipts/<release-or-parameter-sha256>.json` 写入内容哈希绑定的
`telegram-evidence-delivery-receipt-v1`。

- Grid 参数候选要求 BTC/ETH 各三张 PNG，共 6 张：360 天、2026 年 1—2 月、2026 年 5—6 月。
- v22 周模型候选要求 Grid/DCA 四个机器人各三张 PNG，共 12 张。
- 参数与模型更新频道不发送 PDF、模型文件或原始 JSON；文字只保留摘要，证据统一使用手机 PNG。
- 图片缺失、哈希不符、尺寸不是 1440×2400、Telegram 仍在重试或回执事件不匹配时，
  人工审批和 12 小时默认审批都不得通过；当前有效参数/模型继续交易。
- 回执只证明证据已经送达指定频道，不代表候选收益合格，也不替代模型、签名、库存和交易所预检。
...
图片，改发严重告警。图片失败不改变当前仍有效的 v22 周、交易合同或机器人权限，但会
阻断候选模型审批和 Grid 参数激活，直到哈希绑定证据成功送达；不得先激活后补证据。
