...
try:
    from scripts.telegram_notifications import RuntimeErrorChannel, append_event, build_event
except ModuleNotFoundError:
    from live_guard.telegram_notifications import RuntimeErrorChannel, append_event, build_event
...
class LivePortfolioGrid(StrategyV2Base):
    def __init__(self, connectors: Dict[str, ConnectorBase], config: LivePortfolioGridConfig):
        ...
        self._restore_state()
        error_root = Path(self.config.runtime_state_file).parent
        self.runtime_errors = RuntimeErrorChannel(
            event_path=error_root / "telegram_events.jsonl",
            state_path=error_root / "runtime_error_state.json",
            source="grid-live-strategy", strategy="grid",
            bot="grid-live-fdusd-400", pair=",".join(self.config.trading_pairs),
        )
    ...
    def on_tick(self):
        self.runtime_errors.recover_if_quiet(
            "order_submission", quiet_seconds=300,
            trading_status="连续5分钟无新的 Grid 订单拒绝；策略继续按现有风控门运行",
        ...
        try:
            ...
            self.runtime_errors.recovered(
                "strategy_cycle", trading_status="Grid 策略循环正常；当前风控门继续生效",
            )
        except Exception as exc:
            self.logger().error("Live grid cycle failed: %s", exc)
            first = self.first_cycle_failure_at or self.current_timestamp
            self.runtime_errors.failure(
                "strategy_cycle", exc,
                trading_impact=(
                    "本周期未完成，策略自动重试；达到连续失败阈值后才触发既有组合熔断与清仓。"
                ),
                action="retry_then_trip_on_threshold", now=first,
                details={"fail_closed_after_seconds": self.config.fail_closed_seconds},
            )
    ...
    def did_fail_order(self, event: MarketOrderFailureEvent):
        self._forget_order(event.order_id)
        summary = event.error_message or event.error_type or "exchange rejected order"
        self.runtime_errors.failure(
            "order_submission", summary,
            trading_impact="该订单未成交；Grid 继续按现有订单状态和风控规则运行。",
            severity="warning", action="reconcile_order_then_retry_if_allowed",
            details={
                "order_id": str(event.order_id)[:32],
