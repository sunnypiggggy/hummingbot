...
class Guard:
    ...
    def verify_shadow_exchange_ready(self, pairs: list[str]) -> dict:
        ...
        # is fresh and healthy and use only Grid's current owner row.
        inventory_path = self.inventory_ledger.status_path
        if not inventory_path.is_file():
            raise RuntimeError("shared inventory status is missing")
        inventory = json.loads(inventory_path.read_text(encoding="utf-8"))
        if inventory.get("schema") != "account-inventory-status-v2":
            raise RuntimeError("shared inventory status schema is not v2")
        age = time.time() - float(inventory.get("generated_at") or 0)
        if age < -5 or age > 30:
            raise RuntimeError(f"shared inventory status is stale: age={age:.1f}s")
        if not inventory.get("healthy"):
            raise RuntimeError("shared inventory status is unhealthy")
        ownership_coverage = {}
        for pair in pairs:
            asset = pair.split("-")[0]
            ...
            ownership_coverage[pair] = {
                ...
                "source": "shared_account_inventory_v2",
            }
